...
class net(object):
	...
	def __init__(self, proc):
		...
		self.w_and_b = [(10*(np.random.rand(
			self.layer_size[x+1], self.layer_size[x])-0.5), 10*(np.random.rand(
			self.layer_size[x+1], 1)-0.5)) for x in range(NUM_HL+1)]
	...
	def train(self, cycles):
		for cycle in range(int(cycles)):
			...
			del_weights = [np.zeros((self.layer_size[x+1], self.layer_size[x])) for x in range(NUM_HL+1)]
			del_biases = [np.zeros((self.layer_size[x+1], 1)) for x in range(NUM_HL+1)]
			c = 0
			for (picture, label) in self.data_set.data:
				...
				del_weights = map(lambda y, z: y + z, map(lambda x: x[0], changes), del_weights) 
				del_biases = map(lambda y, z: y + z, map(lambda x: x[1], changes), del_biases) 
